...
import logging
from api.order_creation import create_link, form_total_order
...
class FSMEquipmentSelection(StatesGroup):
    ...
    state_30 = State()  
    state_31 = State()  
    state_prot_select = State()  
    state_vfc_selection = State()  
...
async def warning_not_correct_input(message: Message):
    print('state_1')
...
async def enc2res_replacement(callback: CallbackQuery, state: FSMContext):
    ...
    if callback.data == 'yes':
        await callback.message.answer(
            '''⚠️ <b>Обратите внимание</b>! Возможна поддержка
...
async def extra_plates(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        text='''⚠️ <b>Обратите внимание</b>! Необходимо 
использование доп. платы расширения 
для входов и выходов, плата добавлена в набор. (ПВВ1)'''
    )
    await state.update_data(pvv1=True)
...
async def vfc_rmt_sel(callback: CallbackQuery, state: FSMContext):
    print('state_25')
    print(state)
...
async def process_gender_press(callback: CallbackQuery, state: FSMContext):
    print('state_31')
    total_order = form_total_order(user_dict, callback)
    ...
    for key, value in total_order.items():
            if value[1] > 0:
                order_string += str('— <i>' + value[0] + '</i> - ' + str(value[1]) + ' шт.\n')
    await callback.message.answer(
        text=f'''
...
async def confirm_add_note(callback: CallbackQuery, state: FSMContext):
    if callback.data == 'yes':
        ...
        await callback.message.answer(f'''Заказ <b>успешно</b> зарегестрирован!
